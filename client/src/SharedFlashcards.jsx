  ...
  return (
    <>
      ...
      <dialog id="users-dialog-flashcard" className="place-self-center p-4 border border-[var(--text-color)] bg-[var(--bg-color)] text-[var(--text-color)] rounded-xl h-4/5 w-10/12">
                ...
                return (
                  <div className="border border-[var(--text-color)] rounded p-2 flex items-center" key={user.user_id}>
                    ...
                    <div className='flex justify-end flex-col gap-1 '>
                      <button
                        className={`border border-black rounded p-1 ${!isAdded ? 'bg-[var(--button-bg-color)]' : 'bg-[var(--cancel-btn-bg-color)]'} text-[var(--button-text-color)] text-xs md:text-base`}
                        onClick={() => {
                          if (!isAdded) {
                      ...
                      <select
                        ...
                        className={`border rounded p-1 ${isAdded ? 'inline-block' : 'hidden'} text-center bg-[var(--bg-color)] text-xs md:text-sm`}
                      >
        ...
        {/*  Bottom-right toast */}
        {showToast && (
          <dialog className="toast-dialog fixed place-self-end text-xs bottom-4 right-4 bg-[var(--bg-color)] border border-[var(--text-color)] text-[var(--text-color)] px-4 py-2 rounded-xl shadow-lg animate-slide-in z-[9999] flex items-center mb-15 gap-2 md:text-sm" open>
