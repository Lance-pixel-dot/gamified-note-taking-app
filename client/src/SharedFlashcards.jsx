      ...
      <section className={`p-3 pt-0 bg-[var(--bg-color)] flash-container ${props.shareFlashcardsHidden} lg:w-9/12 lg:place-self-end`}>
        <section className="bg-[var(--accent-color)] rounded-b-xl h-full flex flex-col p-4 pt-0 border border-[var(--header-text-color)] border-t-0 lg:border-t lg:rounded-t-xl lg:h-full">
          <section className="flex h-10 items-center">
            ...
            <div className='border border-[var(--header-text-color)] border-l-0 rounded-r-xl h-7 items-center text-[var(--header-text-color)]'>
          ...
          <section id="note-container" className="border-2 border-[var(--header-text-color)] flex-1 overflow-y-auto rounded-xl p-4 flex flex-col gap-2 items-stretch">
            ...
            .map((fc, index) => {
              ...
              return (
                <div key={fc.flashcard_id} className={`border border-[var(--header-text-color)] text-[var(--header-text-color)] rounded-xl p-2 flex items-center gap-2 cursor-pointer ${isReview ? 'bg-[var(--read-color)]' : 'bg-[var(--accent-color)]'}`} 
                onClick={() => {
                ...
                >
                  ...
                  <div className='w-full'>
                    <h2 className="font-bold text-sm md:text-base xl:text-lg">{fc.title}</h2>
                    <span className="text-xs text-[var(tag-color)] italic md:text-sm xl:text-base">Tag: {fc.tag}</span>
            ...
            <button className="border border-[var(--header-text-color)] p-2 rounded-xl text-[var(--header-text-color)] bg-[var(--accent-color)] font-bold w-full flex justify-center items-center gap-2 xl:text-lg" onClick={openShare}> <Icon path={mdiShareVariant} size={1} /> Share Flashcards</button>
          </section>
        </section>
      </section>
            ...
            {flashcards.filter(f =>
              ...
              <div className="border border-[var(--text-color)] rounded p-2 flex justify-center items-center" key={fc.flashcard_id}>
                <div className="flex flex-col gap-2 w-full">
                  <h2 className='font-bold xl:text-lg'>{fc.title}</h2>
                  <span className="text-sm text-[var(--text-color)] italic xl:text-base">Tag: {fc.tag}</span>
                ...
                return (
                  <div className="border border-[var(--text-color)] rounded p-2 flex items-center" key={user.user_id}>
                    ...
                    <div className='flex justify-end flex-col gap-1 '>
                      <button
                        className={`border border-black rounded p-1 ${!isAdded ? 'bg-[var(--button-bg-color)]' : 'bg-[var(--cancel-btn-bg-color)]'} text-[var(--button-text-color)] text-xs md:text-base xl:text-lg`}
                        onClick={() => {
                          if (!isAdded) {
