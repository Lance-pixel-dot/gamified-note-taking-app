...
const Achievements = forwardRef(function Achievements(props, ref) {
  ...
  return (
    <>
      {/*  Bottom-right toast */}
      {showToast && (
        <div className="fixed text-xs bottom-4 right-4 bg-[var(--bg-color)] border border-black-500 border-[var(--text-color)] text-[var(--text-color)] px-4 py-2 rounded-xl shadow-lg animate-slide-in z-[9999] flex items-center gap-2 md:text-sm xl:text-base">
          <Icon path={mdiTrophy} size={1} /> {achievementMessage} +10 <svg className="w-6" viewBox="0 -1.5 48 48" xmlns="http://www.w3.org/2000/">
          ...
          >
            {achievements.map((ach) => {
              ...
              return (
                ...
                >
                  <div>
                    <h3 className="font-semibold text-sm flex gap-2 md:text-base xl:text-lg">
                      {typeEmojis[ach.type] || typeEmojis.default} {ach.name}
                    </h3>
                    <p className="text-xs xl:text-sm">{ach.description}</p>
                  </div>
                  <span
                    className={`text-xs font-bold w-5/12 text-end flex items-center justify-end md:text-sm xl:text-base ${
