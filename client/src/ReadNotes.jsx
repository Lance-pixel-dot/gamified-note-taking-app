...
const ReadNotes = forwardRef(({ note, userId, incrementXP, onCreated, updateCoinsInBackend, markNoteAsRead, api }, ref) => {
  ...
  async function checkIfReadToday() {
    try {
      const res = await fetch(`${api}/read_notes/can-read-note?user_id=${user_id}&note_id=${note.note_id}`);
  ...
  async function handleRead() {
    if (!isRead && incrementXP) {
      try {
        const response = await fetch(`${api}/read_notes/mark-read`, {
          method: "POST",
    ...
    <>
      ...
      >
        <section className="flex flex-col justify-between h-full gap-4">
          ...
          <span className="text-[var(--text-color)] text-xs text-center" >{isRead ? "Read this again tomorrow to earn xp and coins!" : ""}</span>
          <button
            className="border border-black p-2 rounded-xl text-[var(--button-text-color)] bg-[var(--button-bg-color)] font-bold text-sm flex justify-center gap-2"
            onClick={async () => {
              await handleRead(); 
              closeDialog(); 
            }}
          ...
          >
            {isRead ? "Ok" : "Done +3.5xp +2"}
            {isRead ? "" : <svg className="w-6" viewBox="0 -1.5 48 48" xmlns="http://www.w3.org/2000/">
